...
<script src="https://kit.fontawesome.com/bf7086b10a.js"
...
<body>
	...
	<div class="cart outer">
		<div align="center">
			<div class="titleimage" ; align="center" style="position: relative;">
				<img src="resources/myPage/image/mainchar.jpg"
					style="width: 900px; height: 380px;">
				<h3 class="imagetext"
					style="font-size: 34px; font-weight: bold; color: white; position: absolute; top: 63%; left: 48%;">Cart
					&nbsp;</h3>
			</div>
		</div>
		<div id="tablearea" style="margin-top: -60px;">
				...
				<thead>
					<tr>
						...
						<th colspan="3" style="background-color: #fff;">
							<!-- 	                        <button id ="allCkbutton;"; class="view" style="border:white; margin-bottom: 10px; background-color:#EA4F4D; margin-left:-40px; width:90px; height:35px; color:white;">전체선택</button> -->
						</th>
					</tr>
					<tr>
						<!--전체상단 체크박스 ------------------------------------------------>
						<th style="width: 100px; background-color: #EA4F4D;"><input
							type="checkbox" class="all_check_input" name='allCheck'
							style="zoom: 1.5; margin-bottom: -10px;">선택</th>
						<th style="background-color: #EA4F4D;" co>이미지</th>
						<th style="background-color: #EA4F4D;">상품명</th>
						<th style="background-color: #EA4F4D;">가격</th>
						<th style="background-color: #EA4F4D; width: 170px;">수량</th>
					...
					<c:forEach items="${cList}" var="c">
						<tr>
							...
							<td class="cart_info_td">
								<!--개별체크박스 ----------------------------------------------> <input
								type="checkbox" class="individual_cart_checkbox" name="RowCheck"
								value="${c.goodsNo}" style="zoom: 1.3;" checked="checked">
								<input type="hidden" class="individual_price_input"
								value="${c.goodsPrice}"> <input type="hidden"
								class="individual_count_input" value="${c.quantity}"> <input
								type="hidden" class="individual_totalPrice_input"
								value="${c.goodsPrice*c.quantity}">
							</td>
							<td><img src="${c.goodsAttachChangeName}" alt="img"
								class="wishimg" style="width: 100px; height: 100px;"></td>
							<td>
								<div>상품명:${c.goodsName}</div>
							</td>
							<td>
								<div>
									판매가: <span><fmt:formatNumber value="${c.goodsPrice}"
											pattern="#,###원" /></span>
								</div>
							</td>
							<td>
								<div class="countbuttonarea">
									<div>
										<input type="hidden" name="" value="${c.cartNo }"> <input
											type="number" id="inputquantity"
											style="width: 40px; margin-bottom: 3px; margin-left: 5px;"
											min="1" max="99" step="1" value="${c.quantity }">
										<%--<input type="text" id="inputquantity" value="${c.quantity}" style="width:40px;margin-bottom:3px;margin-left:5px;	" size="2"> --%>
										&nbsp;
										<button type="button" class="quantityBtn" id="modifybtn">변경</button>
									</div>
									<%--<a class="quantity_modify_btn" onclick="modifybtn('${c.cartNo}')" value="${c.cartNo}">변경</a> --%>
								</div>
							</td>
							<td>합계: <span><fmt:formatNumber
										value="${c.goodsPrice*c.quantity}" pattern="#,###원" /></span>
							</td>
							<td>
								<div>
									<button class="cartview"
										style="margin-bottom: 10px; background-color: #EA4F4D; color: #eee; width: 70px;">주문</button>
								</div>
								<div>
									<button class="cartdelete" onclick="deleteOk('${c.cartNo}');"
										style="background-color: #EA4F4D; color: #eee; width: 70px;">삭제</button>
								</div>
							</td>
						</tr>
					</c:forEach>
		...
		<div id="totalpricearea" align="center">
			<table class=".carttable">
				...
				<tbody>
					<tr>
						<td class="totalCount_td">0개</td>
						<td class="totalPrice_td">0원</td>
						<td>+</td>
						<td class="deliveryPrice_td">0원</td>
						<td>=</td>
						<td class="finalTotalPrice_td">0원</td>
					</tr>
				</tbody>
			</table>
		</div>
		...
		<form action="cart/update" method="post" class="quantity_update_form">
			<input type="hidden" name="cartNo" class="update_cartNo"> <input
				type="hidden" name="quantity" class="update_quantity"> <input
				type="hidden" name="memberNo" value="${loginUser.memberNo}">
		</form>

		<br>
		<br>
		<div id="buttonarea" align="center">
			<button class="wishview"
				style="margin-bottom: 10px; background-color: #EA4F4D; color: white; height: 40px; font-weight: bold; width: 120px; border-color: white;">주문하기</button>
		</div>
		...
		<script type="text/javascript">
			...
			function setTotalInfo() {
				...
				//배송비결정 총가격이 50000이상일때만 0원,그 이하는 3000
				if (totalPrice >= 50000) {
					deliveryPrice = 0;
			...
			//삭제영역--------------------------------------------------------------
			function deleteOk(cartNo) {
				if (confirm("해당 상품을 삭제하시겠습니까?")) {
					location.href = "deleteCart.my?cartNo=" + cartNo;
